...
1)  Когда вы запрашиваете доменное имя, ваш компьютер сначала проверяет свой локальный кэш, 
чтобы узнать, не искали ли вы адрес ранее; если нет, будет сделан запрос к вашему рекурсивному DNS-серверу.


2) Рекурсивный DNS-сервер обычно предоставляется вашим интернет-провайдером, но вы также можете выбрать свой собственный. 
...
1) Корневые серверы выступают в качестве DNS-основы Интернета; их работа заключается в том, чтобы перенаправить вас на нужный сервер домена верхнего уровня, 
...
2) Сервер TLD хранит записи о том, где найти авторитативный сервер для ответа на запрос DNS. 
...
Вы часто найдете несколько серверов имен для доменного имени, которые могут выступать в качестве резерва на случай выхода из строя одного из них.
 ...
 3) Авторитативный DNS-сервер — это сервер, который отвечает за хранение DNS-записей для конкретного доменного имени и на котором будут вноситься любые изменения в DNS-записи
